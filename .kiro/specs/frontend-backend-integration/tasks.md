...
- [ ] 1. Infraestrutura base — Client Frontend
  - [ ] 1.1 Criar `src/lib/api.ts` com `decodeShopId()`, `clientApi()` e `ApiError`
  ...
  - [ ] 1.3 Criar `src/lib/price.ts` com `centsToReais()` e `reaisToCents()`
  ...
  - [ ] 1.5 Criar `.env` e `.env.example` no `trinity-scheduler-client`
    - `.env`: `VITE_API_URL=http://localhost:3000` e `VITE_SHOP_ID=`
    - `.env.example`: mesmas chaves com valores de exemplo
    - _Requirements: 1.1_

- [ ] 2. Infraestrutura base — Admin Frontend
  - [ ] 2.1 Criar `src/lib/api.ts` com `adminApi()` e `ApiError` no `trinity-scheduler-admin`
  ...
  - [ ] 2.3 Criar `src/utils/price.ts` com `centsToReais()` e `reaisToCents()` no `trinity-scheduler-admin`
  ...
  - [ ] 2.5 Criar `src/lib/mappers.ts` com `fromBackendProfessional()` e `toBackendProfessional()` no `trinity-scheduler-admin`
  ...
  - [ ] 2.7 Criar `.env` e `.env.example` no `trinity-scheduler-admin`
    - `.env`: `VITE_API_URL=http://localhost:3000`
    - `.env.example`: mesma chave com valor de exemplo
    - _Requirements: 2.1_

- [ ] 3. Checkpoint — Infraestrutura base
...
- [ ] 4. Stores de autenticação
  - [ ] 4.1 Modificar `src/stores/authStore.ts` no `trinity-scheduler-client`
  ...
  - [ ] 4.3 Modificar `src/stores/userStore.ts` no `trinity-scheduler-admin`
...
- [ ] 5. Serviços de autenticação — Client Frontend
  - [ ] 5.1 Modificar `src/services/authService.ts` no `trinity-scheduler-client`
...
- [ ] 6. Serviços de dados — Client Frontend
  - [ ] 6.1 Modificar `src/services/serviceService.ts` no `trinity-scheduler-client`
    - `getServices()` → `GET /services` via `clientApi`, aplica `centsToReais` no campo `price`
    - Remover importações de mocks
    - _Requirements: 5.1, 5.3, 9.3, 18.1, 18.3_

  - [ ] 6.2 Criar `src/services/addonService.ts` no `trinity-scheduler-client`
    - `getAddons()` → `GET /addons` via `clientApi`, aplica `centsToReais` no campo `price`
    - _Requirements: 5.2, 5.3, 5.5, 9.3_

  - [ ] 6.3 Modificar `src/services/professionalService.ts` no `trinity-scheduler-client`
    - `getProfessionals()` → `GET /professionals` via `clientApi`
    - Remover importações de mocks
    - _Requirements: 6.1, 18.1, 18.3_

  - [ ] 6.4 Modificar `src/services/availabilityService.ts` no `trinity-scheduler-client`
  ...
  - [ ] 6.5 Modificar `src/services/appointmentService.ts` no `trinity-scheduler-client`
...
- [ ] 8. Serviços de agendamentos e clientes — Admin Frontend
  - [ ] 8.1 Modificar `src/services/appointmentService.ts` no `trinity-scheduler-admin`
  ...
  - [ ] 8.2 Modificar `src/services/clientService.ts` no `trinity-scheduler-admin`
...
- [ ] 9. Serviços de catálogo e equipe — Admin Frontend
  - [ ] 9.1 Modificar `src/services/serviceService.ts` no `trinity-scheduler-admin`
  ...
  - [ ] 9.2 Modificar `src/services/staffService.ts` no `trinity-scheduler-admin`
  ...
  - [ ] 9.3 Modificar `src/services/unitService.ts` no `trinity-scheduler-admin`
...
- [ ] 10. Serviços de configuração e dashboard — Admin Frontend
  - [ ] 10.1 Criar `src/services/shopService.ts` no `trinity-scheduler-admin`
  ...
  - [ ] 10.2 Criar `src/services/dashboardService.ts` no `trinity-scheduler-admin`
  ...
  - [ ] 10.3 Modificar `src/stores/shopStore.ts` no `trinity-scheduler-admin`
...
- [ ] 11. Hooks do Admin Frontend
  - [ ] 11.1 Modificar `src/hooks/useClients.ts` no `trinity-scheduler-admin`
    - Passar `search`, `page` e `perPage` como query params para `clientService.search()` — não filtrar localmente
    - _Requirements: 13.1_

  - [ ] 11.2 Modificar `src/hooks/useStaff.ts` no `trinity-scheduler-admin`
    - Passar `unitId` como query param para `staffService.getAll({ unitId })` — não filtrar localmente
    - _Requirements: 11.1_

  - [ ] 11.3 Modificar `src/hooks/useAppointments.ts` no `trinity-scheduler-admin`
